...
MANGOHUD_LOG_DIR = pathlib.Path.home() / "Documents" / "MangoLogs"
...
MANGOHUD_CONF_PATHS = [
    ...
    pathlib.Path.home() / ".var/app/com.valvesoftware.Steam/config/MangoHud/MangoHud.conf",  # Steam Flatpak
]
...
_LOGGING_VALS = {
    ...
    **BOTTLENECK_KEYS,
    "cpu_temp": 1, "cpu_power": 1, "cpu_mhz": 1,
    "gpu_stats": 1, "gpu_temp": 1, "gpu_power": 1,
    "gpu_mem_clock": 1, "gpu_mem_temp": 1,
    "vram": 1, "ram": 1, "swap": 1,
    ...
    if args.game:
        ...
        if found:
            ...
            if added:
                ...
                print(f"    Added missing keys for bottleneck analysis: {', '.join(added)}")
            else:
...
def _run_mangoplot(csv_path: pathlib.Path, out_dir: pathlib.Path) -> int:
    ...
    try:
        result = subprocess.run(cmd, cwd=str(out_dir), capture_output=True, text=True, timeout=60)
        ...
        log.error("mangoplot not found in PATH."); return 1
    except subprocess.TimeoutExpired:
        log.error("mangoplot timed out after 60s."); return 1
...
def cmd_graph(args: argparse.Namespace) -> int:
    ...
    if not _MPL:
        log.error("Neither mangoplot nor matplotlib available.\n"
...
def cmd_organize(args: argparse.Namespace) -> int:
    ...
    if not dry:
        for game_dir in sorted(dest.iterdir()):
            ...
            non_link_csvs = [f for f in game_dir.glob("*.csv")
        ...
        if src_dir.is_dir():
            for gd in sorted(src_dir.iterdir()):
                ...
                if cur.is_symlink() or cur.exists():
                    ...
                    latest = sorted([f for f in gd.glob("*.csv")], key=lambda p: p.stat().st_mtime)
                    if latest:
                        csvs.append(latest[-1])
    return csvs
...
def cmd_upload(args: argparse.Namespace) -> int:
    ...
    if not session:
        log.error("No session token provided. Use --session TOKEN or set "
                  "FLIGHTLESS_SESSION environment variable.\n"
                  "  To get your session token:\n"
                  "    1. Log in at %s\n"
                  "    2. Open browser DevTools -> Application -> Cookies\n"
                  "    3. Copy the 'mysession' cookie value", FLIGHTLESS_BASE)
        return 1
    ...
    game = getattr(args, "game", None)
    title = args.title or f"Benchmark: {game or 'All Games'} - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pg.add_argument("--matplotlib", action="store_true",
    ...
    pu = sub.add_parser("upload",
        ...
        """),
        formatter_class=argparse.RawDescriptionHelpFormatter)
    pu.add_argument("-s", "--session", metavar="TOKEN",
                    help="FlightlessSomething 'mysession' cookie value. "
                         "Also reads FLIGHTLESS_SESSION env var.")
    pu.add_argument("-g", "--game", metavar="NAME",
                    help="Upload only logs for this game.")
    pu.add_argument("-i", "--input", nargs="*", metavar="PATH",
                    help="Specific CSV file(s) or directories to upload.")
    pu.add_argument("--source", metavar="DIR", default=str(BENCH_LOG_DIR),
                    help=f"Source directory for organized logs (default: {BENCH_LOG_DIR}).")
    pu.add_argument("--title", metavar="TEXT",
                    help="Benchmark title (default: auto-generated).")
    pu.add_argument("--description", metavar="TEXT",
                    help="Benchmark description (default: auto-generated).")
    pu.add_argument("--url", metavar="URL",
